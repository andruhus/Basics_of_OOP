\label{_g_mock_for_dummies_md_lib_googletest-master_googlemock_docs_for_dummies}%
\Hypertarget{_g_mock_for_dummies_md_lib_googletest-master_googlemock_docs_for_dummies}%
\hypertarget{_g_mock_for_dummies_autotoc_md97}{}\doxysection{What Is g\+Mock?}\label{_g_mock_for_dummies_autotoc_md97}
...
\end{DoxyEnumerate}\hypertarget{_g_mock_for_dummies_autotoc_md98}{}\doxysection{Why g\+Mock?}\label{_g_mock_for_dummies_autotoc_md98}
...
\end{DoxyItemize}\hypertarget{_g_mock_for_dummies_autotoc_md99}{}\doxysection{Getting Started}\label{_g_mock_for_dummies_autotoc_md99}
g\+Mock is bundled with googletest.\hypertarget{_g_mock_for_dummies_autotoc_md100}{}\doxysection{A Case for Mock Turtles}\label{_g_mock_for_dummies_autotoc_md100}
...
You don\textquotesingle{}t need to define these mock methods somewhere else -\/ the {\ttfamily M\+O\+C\+K\+\_\+\+M\+E\+T\+H\+OD} macro will generate the definitions for you. It\textquotesingle{}s that simple!\hypertarget{_g_mock_for_dummies_autotoc_md103}{}\doxysubsection{Where to Put It}\label{_g_mock_for_dummies_autotoc_md103}
...
So, the rule of thumb is\+: if you need to mock {\ttfamily Foo} and it\textquotesingle{}s owned by others, define the mock class in {\ttfamily Foo}\textquotesingle{}s package (better, in a {\ttfamily testing} sub-\/package such that you can clearly separate production code and testing utilities), put it in a {\ttfamily .h} and a {\ttfamily cc\+\_\+library}. Then everyone can reference them from their tests. If {\ttfamily Foo} ever changes, there is only one copy of {\ttfamily \mbox{\hyperlink{class_mock_foo}{Mock\+Foo}}} to change, and only tests that depend on the changed methods need to be fixed.

Another way to do it\+: you can introduce a thin layer {\ttfamily Foo\+Adaptor} on top of {\ttfamily Foo} and code to this new interface. Since you own {\ttfamily Foo\+Adaptor}, you can absorb changes in {\ttfamily Foo} much more easily. While this is more work initially, carefully choosing the adaptor interface can make your code easier to write and more readable (a net win in the long run), as you can choose {\ttfamily Foo\+Adaptor} to fit your specific domain much better than {\ttfamily Foo} does.\hypertarget{_g_mock_for_dummies_autotoc_md104}{}\doxysection{Using Mocks in Tests}\label{_g_mock_for_dummies_autotoc_md104}
...
Admittedly, this test is contrived and doesn\textquotesingle{}t do much. You can easily achieve the same effect without using g\+Mock. However, as we shall reveal soon, g\+Mock allows you to do {\itshape so much more} with the mocks.\hypertarget{_g_mock_for_dummies_autotoc_md105}{}\doxysection{Setting Expectations}\label{_g_mock_for_dummies_autotoc_md105}
...
right.\char`\"{}\hypertarget{_g_mock_for_dummies_autotoc_md106}{}\doxysubsection{General Syntax}\label{_g_mock_for_dummies_autotoc_md106}
...
This works for all non-\/overloaded methods; if a method is overloaded, you need to help g\+Mock resolve which overload is expected by specifying the number of arguments and possibly also the \href{cook_book.md\#SelectOverload}{\texttt{ types of the arguments}}.\hypertarget{_g_mock_for_dummies_autotoc_md108}{}\doxysubsection{Cardinalities\+: How Many Times Will It Be Called?}\label{_g_mock_for_dummies_autotoc_md108}
...
{\bfseries{Quick quiz\+:}} what do you think will happen if a function is expected to be called twice but actually called four times?\hypertarget{_g_mock_for_dummies_autotoc_md109}{}\doxysubsection{Actions\+: What Should It Do?}\label{_g_mock_for_dummies_autotoc_md109}
...
Instead of returning 100, 101, 102, ..., consecutively, this mock function will always return 100 as {\ttfamily n++} is only evaluated once. Similarly, {\ttfamily Return(new Foo)} will create a new {\ttfamily Foo} object when the {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+C\+A\+L\+L()} is executed, and will return the same pointer every time. If you want the side effect to happen every time, you need to define a custom action, which we\textquotesingle{}ll teach in the \href{http://<!-- GOOGLETEST_CM0012 DO NOT DELETE --}{\texttt{ cook book}}.

\mbox{\hyperlink{class_time}{Time}} for another quiz! What do you think the following means?
